...
const stopCommunication = (reason) => {
    ...
    updateStatus(`comm: stopped (${reason})`);
    console.log("[attack] communication stopped", reason);
};
...
const sendHeartbeat = () => {
    ...
    const url = `/assets/heavy_10.jpg?hb=${Date.now()}`;
    fetch(url, { cache: "no-store", signal })
        .then(() => {
            if (isStopped) {
                return;
            }
            updateStatus("comm: running (heartbeat)");
            console.log("[attack] heartbeat sent", url);
        })
        .catch((error) => {
            // abort由来の失敗は期待動作なので別ログにする
            if (error && error.name === "AbortError") {
                console.log("[attack] heartbeat aborted", url);
                return;
            }
            console.log("[attack] heartbeat failed", error);
        })
        .finally(() => {
